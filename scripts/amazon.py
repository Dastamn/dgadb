...
for product in ds.keys():
    ...
    unique_ids = pl.concat(
        [df["user_id"], df["parent_asin"]]).unique().to_frame("str_id")
    unique_ids = unique_ids.with_row_index(name="int_id")

    df = df.join(unique_ids, left_on="user_id", right_on="str_id").drop(
        ["user_id"]).rename({"int_id": "src"})
    df = df.join(unique_ids, left_on="parent_asin", right_on="str_id").drop(
        ["parent_asin"]).rename({"int_id": "tgt"})
    ...
    df_edge_features_num = df.select(
        ["edge_id", "helpful_vote", "rating"])  # is rating cat or num?
    ...
    df = df.join(unique_ids, left_on="parent_asin", right_on="str_id").drop(
        ["parent_asin"]).rename({"int_id": "node_id"})

    df_node_features_str = df.select(
        ["node_id", "main_category", "title", "features", "description", "store", "categories", "details"])
