...
parser.add_argument(
    "--max-train-num", type=int, default=100000, help="set maximum number of train links (to fit into memory)"
)
parser.add_argument("--no-cuda", action="store_true", default=False, help="disables CUDA training")
parser.add_argument("--seed", type=int, default=1, metavar="S", help="random seed (default: 1)")
parser.add_argument("--test-ratio", type=float, default=0.815, help="ratio of test links")
...
parser.add_argument("--max-nodes-per-hop", default=None, help="if > 0, upper bound the # nodes per hop by subsampling")
parser.add_argument(
    "--use-embedding", action="store_true", default=False, help="whether to use node2vec node embeddings"
)
...
if args.train_name is None:
    ...
    net = ssp.csc_matrix(
        (np.ones(len(train_idx)), (train_idx[:, 0], train_idx[:, 1])), shape=(max_idx + 1, max_idx + 1)
    )
    ...
    train_pos, train_neg, test_pos, test_neg = sample_dyn(
        net, train_pos=train_pos, test_pos=test_pos, max_train_num=args.max_train_num
    )
...
for epoch in range(cmd_args.num_epochs):
    ...
    print(
        (
            "\033[93maverage test of epoch %d: loss %.5f acc %.5f auc %.5f avg_precision %.5f precision-recall auc %.5f\033[0m"
