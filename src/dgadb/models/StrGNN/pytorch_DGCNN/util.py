...
cmd_opt.add_argument(
    "-printAUC", type=bool, default=False, help="whether to print AUC (for binary classification only)"
)
...
def load_data():
    ...
    if cmd_args.test_number == 0:
        train_idxes = np.loadtxt(
            "data/%s/10fold_idx/train_idx-%d.txt" % (cmd_args.data, cmd_args.fold), dtype=np.int32
        ).tolist()
        test_idxes = np.loadtxt(
            "data/%s/10fold_idx/test_idx-%d.txt" % (cmd_args.data, cmd_args.fold), dtype=np.int32
        ).tolist()
