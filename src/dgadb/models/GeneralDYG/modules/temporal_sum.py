...
class TemporalSumLayer(torch.nn.Module):
    ...
    def __init__(self, n_node_features, n_edge_features, time_dim, output_dimension):
    ...
    def forward(
        self, node_feature, edge_index, edge_feature, src_time_features, edge_time, mask=None, sample_ratio=None
    ):
        """
        无向图，边增加为双向
        增加自闭环
        ...
        if mask is not None and sample_ratio is None:
            edge_index, edge_feature, src_time_features, edge_time = self.mask_edge(
                edge_index, edge_feature, src_time_features, edge_time, mask
            )

        if self.reverse_flag:
            edge_index, edge_feature, src_time_features, edge_time, sample_ratio = self.reverse_edge(
                edge_index, edge_feature, src_time_features, edge_time, sample_ratio
            )
        if self.selfloop_flag:
            edge_index, edge_feature, src_time_features, edge_time, sample_ratio = self.add_selfloop(
                node_feature, edge_index, edge_feature, src_time_features, edge_time, sample_ratio
            )
        node_i = edge_index[:, 0]
        node_j = edge_index[:, 1]
    ...
    def add_selfloop(self, node_feature, edge_index, edge_feature, src_time_features, edge_time, sample_ratio):
        ...
        edge_feature = torch.cat(
            [
                edge_feature,
                torch.zeros(
                    [node_id.shape[0], edge_feature.shape[1]], dtype=edge_feature.dtype, device=edge_feature.device
                ),
            ],
            dim=0,
        )
        ...
        if sample_ratio is not None:
            sample_ratio = torch.cat(
                [sample_ratio, torch.ones([node_id.shape[0]], dtype=sample_ratio.dtype, device=sample_ratio.device)]
            )
