...
class TransformerBinaryClassifier(nn.Module):
    def __init__(self, config, device, hidden_size=128):
        ...
        self.encoder_layer = CustomTransformerEncoderLayer(
            d_model=self.input_size, nhead=config.n_heads, dropout=config.drop_out, dim_feedforward=hidden_size
        )
    ...
    def forward(self, raw_input, GNN_output, mask):
        ...
        logits = self.classifier(mean_output).squeeze()
        # logits = self.sigmoid(logits).squeeze() Shouldnt do sigmoid here
