...
if __name__ == "__main__":
    ...
    args = args_parser()
    custom_base = os.path.join("custom", args.dataset)
    if os.path.exists(custom_base):
        print(f"Detected custom dataset folder: {args.dataset}")
        parts = args.dataset.split("_")
        if len(parts) >= 3:
            args.anomaly_ratio = parts[-1]
            args.train_ratio = parts[-2]
    ...
    # log_path = 'logs/custom_results/' if os.path.exists(custom_base) else f'logs/{args.dataset}/'
    if not os.path.exists(log_path):
        os.makedirs(log_path)

    log_file = os.path.join(log_path, f"{args.anomaly_ratio}")
    # log_file = os.path.join(log_path, f'{args.dataset}.log')
    log = open(log_file, "a")
    ...
    for epoch in tqdm(range(args.epochs)):
        ...
        if (epoch + 1) % 5 == 0 or epoch == args.epochs - 1:
            ...
            tb.add_scalar("auc_all", auc_all, epoch)
