...
def load_custom_csv(dataset_name, snap_size, x_dim, device):
    # Construct the path based on your description
    # Note: You might want to pass these ratios as args
    base_path = f"custom/{dataset_name}"
    ...
    # Convert to numpy
    # train: [src, tgt, timestamps] -> we need [src, tgt, 1] (1 for normal)
    train_edges = train_df.iloc[:, 0:2].values
    train_data = np.zeros((len(train_edges), 3))
    train_data[:, 0:2] = train_edges
    train_data[:, 2] = 1  # Assuming training is all normal/noise-filtered

    # test: [src, tgt, timestamps, labels] -> we need [src, tgt, labels]
    test_edges = test_df.iloc[:, 0:2].values
    test_labels = test_df.iloc[:, 3].values
    test_data = np.zeros((len(test_edges), 3))
    test_data[:, 0:2] = test_edges
    test_data[:, 2] = test_labels

    # Flip labels
    test_data[:, 2] = 1 - test_df.iloc[:, 3].values

    # Combine all edges for Node2Vec to get global embeddings
    all_edges = np.concatenate([train_edges, test_edges], axis=0)

    # Re-index nodes to 0...N-1
    unique_nodes, edges_indexed = np.unique(all_edges, return_inverse=True)
    edges_indexed = edges_indexed.reshape(all_edges.shape)

    n = len(unique_nodes)
    m = len(all_edges)

    # Split back to train/test indexed edges
    train_indexed = edges_indexed[: len(train_edges)]
    test_indexed = edges_indexed[len(train_edges) :]

    train_data[:, 0:2] = train_indexed
    test_data[:, 0:2] = test_indexed

    # Generate Node2Vec features
    print("Training Node2Vec for features...")
    x = n2v_train(edges_indexed, x_dim, device, n, epoch_num=50)

    # Process into snapshots

    def create_snapshots(data_matrix, node_features):
        snaps = []
        num_snaps = int(len(data_matrix) / snap_size + 0.5)
        for i in range(num_snaps):
            start = i * snap_size
            end = (i + 1) * snap_size
            if start >= len(data_matrix):
                break

            slice_data = data_matrix[start:end]
            edge_idx = torch.LongTensor(slice_data[:, 0:2]).t().contiguous()
            y = torch.LongTensor(slice_data[:, 2])
            node_idx = torch.unique(edge_idx)
            edge_attr = torch.randn(edge_idx.size(1), 64)

            data = Data(x=node_features, edge_index=edge_idx, node_index=node_idx, y=y, edge_attr=edge_attr)
            snaps.append(data)
        return snaps

    train_snaps = create_snapshots(train_data, x)
    test_snaps = create_snapshots(test_data, x)

    return train_snaps + test_snaps, len(train_snaps)


def generateDataset(dataset, raw_file, device, snap_size, train_per, anomaly_per, noise_ratio, x_dim):
    custom_path = f"custom/{dataset}"
    if os.path.exists(custom_path):
        print(f"Loading custom dataset from {custom_path}")
        return load_custom_csv(dataset, snap_size, x_dim, device)
...
def anomaly_generation2(ini_graph_percent, anomaly_percent, noise_ratio, data, n, m, seed=1):
    ...
    print(
        "[%s] initial network edge percent: %.2f, anomaly percent: %.2f.\n"
        % (datetime.datetime.now(), ini_graph_percent, anomaly_percent)
    )
    t0 = time.time()
