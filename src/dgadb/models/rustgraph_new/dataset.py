...
class DynamicGraphAnomaly(InMemoryDataset):
    url = {
        "email": "https://nrvis.com/download/data/dynamic/email-dnc.zip",
        ...
        "hepth": [
            ...
            "https://snap.stanford.edu/data/cit-HepTh-dates.txt.gz",
        ],
        "enron": "",
    }
    ...
    def __init__(self, root, name, args, transform=None, pre_transform=None, pre_filter=None):
        ...
        super(DynamicGraphAnomaly, self).__init__(
            root=root, transform=transform, pre_transform=pre_transform, pre_filter=pre_filter
        )
    ...
    def download(self):
        if os.path.exists(os.path.join("custom", self.dataset)):
            print("Custom dataset detected. Skipping download.")
            return
    ...
    def process(self):
        ...
        raw_file = os.path.join(self.raw_dir, self.raw_file_names[0]) if self.dataset in self.url else ""
        data_list, train_size = generateDataset(
            self.dataset,
            raw_file,
            self.device,
            self.snap_size,
