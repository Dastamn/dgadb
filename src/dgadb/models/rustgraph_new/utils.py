    ...
    for i, ax in enumerate(axs.flat):
        ...
        if i == 0:
            ...
            for j, clustering_coeffs in enumerate(dataset_clustering_coeffs):
                ...
                # Use a different marker for each curve
                marker = markers[j % len(markers)]
                color = curve_colors[j % len(curve_colors)]
                ax.plot(
                    range(min(len(clustering_coeffs), 50)),
                    clustering_coeffs[:50],
                    label=dsnames[j],
                    linestyle=line_style,
                    color=color,
                )
                ax.scatter(
                    [0, len(clustering_coeffs[:50]) - 1],
                    [clustering_coeffs[0], clustering_coeffs[len(clustering_coeffs[:50]) - 1]],
                    marker=marker,
                    color=color,
                    label=None,
                )
        ...
        # Plot the density curves for each dataset
            ...
            for j, densities in enumerate(dataset_densities):
                ...
                color = curve_colors[j % len(curve_colors)]
                ax.plot(
                    range(min(len(densities), 50)), densities[:50], label=dsnames[j], linestyle=line_style, color=color
                )
                ax.scatter(
                    [0, len(densities[:50]) - 1],
                    [densities[0], densities[len(densities[:50]) - 1]],
                    marker=marker,
                    color=color,
                    label=None,
                )
        ...
        # Plot the transitivity curves for each dataset
            ...
            for j, transitivity in enumerate(dataset_transitivity):
                ...
                ax.scatter(
                    [0, len(transitivity[:50]) - 1],
                    [transitivity[0], transitivity[len(transitivity[:50]) - 1]],
                    marker=marker,
                    color=color,
                    label=None,
                )
        ...
        # Plot the Connected Components curves for each dataset
            ...
            for j, connected_components in enumerate(dataset_connected_components):
                ...
                color = curve_colors[j % len(curve_colors)]
                ax.plot(
                    range(min(len(connected_components), 50)),
                ...
                ax.scatter(
                    [0, len(connected_components[:50]) - 1],
                    [connected_components[0], connected_components[len(connected_components[:50]) - 1]],
                    marker=marker,
                    color=color,
                    label=None,
                )
...
def visualize_topology(datasets):
    # Iterate over each dataset
    # 'UCI Messages', 'Bitcoin-Alpha', 'Bitcoin-OTC', 'Emain-DNC'
    dsnames = ["Digg", "AS-Topology"]
