...
parser.add_argument('--lr', type=float, default=0.0008, help='Initial learning rate.')  # 0.0005
...
parser.add_argument('--w', type=int, default=2, help='Hyper-parameters in the score function.')
...
# Load data
data_path = 'src/dgadb/models/ADDGRAPH/UCI_D_Addgraph/opsahl-ucsocial/out.opsahl-ucsocial'
...
N_S = negative_sample()
# optimizer = optim.Adam([{'params' : Net1.parameters(), 'lr' : args.lr_}, {
#     'params' : itertools.chain(Net2.parameters(), Net3.parameters(), Net4.parameters()), 'lr' : args.lr}])  # weight_decay=args.weight_decay
# optimizer = optim.SGD([{'params' : Net1.parameters(), 'lr' : args.lr_}, {
#     'params' : itertools.chain(Net2.parameters(), Net3.parameters(), Net4.parameters()), 'lr' : args.lr}])  # weight_decay=args.weight_decay
...
optimizer2 = optim.Adam(itertools.chain(Net2.parameters(), Net3.parameters(), Net4.parameters()),
                      lr=args.lr,
                      )  # weight_decay=args.weight_decay
...
# snapshots=np.load("src/dgadb/models/ADDGRAPH/UCI_D_Addgraph/framwork/snapshot_10a.npz", allow_pickle=True)
...
def train():
    ...
    for epoch in range(args.epochs):
        ...
        H_list = torch.zeros(1, nodes, args.hidden) # shape: (1, num_nodes, num_units)
        H_ = torch.zeros((args.w, nodes, args.hidden)) # (w_size, num_nodes, num_units)
        ...
        # here H_list should be (2, num_nodes, num_units)

        stdv = 1. / math.sqrt(H_list[-1].size(1)) # 1 / sqrt(num_units) = 1 / 10 = 0.1

        # H_list[-1].data.uniform_(-stdv, stdv)
        # H_list[-1] = torch.rand(nodes, args.hidden)
        # nn.init.sparse_(H_list[-1].t(), sparsity=0.9)
        H_list[-1][:n_train, :].data.uniform_(-stdv, stdv) # init of training node features

        U_adj = torch.zeros((nodes, nodes)) # adj matrix

        loss_a = torch.zeros(1) # loss

        for i in range(l_train): # number of train snapshots is 6
            optimizer1.zero_grad()
            optimizer2.zero_grad()
            # snapshot=snapshots_train[i]
            snapshot = torch.from_numpy(snapshots_train[i]) # get edge snapshot (snapshot_size, 2)

            H = H_list[-1] # get current node features, this includes both train and test ?


            # Populate the short-term window for the attention mechanism
            for j in range(args.w):
                H_[j] = H_list[-args.w + j]

            # Get the graph structure for the current snapshot
            U_adj, adj, Adj = update_adj(U_adj=U_adj, snapshot=snapshot, nodes=nodes) # update adjacency with current snapshot
            ...
            # short = Net2(C=H_)
            # Hn = Net3(current=current, short=short)
            H_list = torch.cat([H_list, Hn.unsqueeze(0)], dim=0) # Before was (2, num_nodes, num_units)
            print("new h list shape", H_list.shape) # Now is 3 and so on (1st iter)

            # H_list.append(Hn)
            # n_data,n_loss=negative_sample(adj=adj, snapshot=snapshot, H=Hn, f=Net4)
            n_loss = N_S(adj=adj, U_adj=U_adj, snapshot=snapshot, H=Hn, f=Net4, arg=args.cuda)
            ...
            loss_a = loss_a + loss1 / (l_train) + loss2 / (lens * l_train)
            ...
            # print("第%d个epoch的学习率：%f" % (epoch, optimizer1.param_groups[0]['lr']))
            # print("第%d个epoch的学习率：%f" % (epoch, optimizer2.param_groups[0]['lr']))
            # print("第%d个epoch的学习率：%f" % (epoch, optimizer.param_groups[3]['lr']))
        ...
        print(time.time() - t)
        # writer1 = SummaryWriter('runs/S5_Adam_w3_loss')
        # writer1 = SummaryWriter('runs/Adam_w1_loss')
        ...
        state = {'Net1': Net1.state_dict(), 'Net2':
            Net2.state_dict(), 'Net3': Net3.state_dict(),
                ...
                'H_list': H_list, 'loss_a': loss_a, }
        # # print(n_loss)
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        dir = './checkpoint/S53_Adam_lr_0.0008_5_w2_h1_epoch{}.pth'.format(epoch)
    ...
    U_adj = {'U_adj': U_adj}
    dir = './checkpoint/U_adj.pth'
    # torch.save(U_adj, dir)
    print('Finish')
