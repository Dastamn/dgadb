...
parser.add_argument('--lr_', type=float, default=0.0008, help='Initial learning rate.')  # 0.0005
...
def train():
    ...
    for epoch in range(args.epochs):
        ...
        for i in range(l_train):
            ...
            # tail = np.max(tail_)
            # nn.init.sparse_(H_list[-1][head:tail, :].t(), sparsity=0.8)
            # head = tail
        ...
        if not os.path.isdir('checkpoint'):
            os.mkdir('checkpoint')
        dir = './checkpoint/h4_Sparse_S17_Adam_lr_0.001_8_epoch{}.pth'.format(epoch)
