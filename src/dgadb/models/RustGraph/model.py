...
class GConv(nn.Module):
    def __init__(self, input_dim, output_dim, device, act=None, bias=True, dropout=0.0):
...
class Model(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.encoder = Generative(x_dim, h_dim, z_dim, layer_num, self.device)
        self.contrastive = Contrastive(self.device, z_dim, window)
    ...
    def forward(self, graph: TemporalGraph, split="train", accumulate=True, y_rect=None, h_t=None):
        ...
        for t, snapshot in enumerate(snapshot_loader):
            ...
            # collect labels for this snapshot
