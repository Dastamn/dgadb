...
def get_neighbor_finder(data, uniform=False, max_node_idx=None):
    max_node_idx = max(data.sources.max(), data.destinations.max()
                       ) if max_node_idx is None else max_node_idx
...
class NeighborFinder:
    ...
    @classmethod
    def from_temporal_graph(cls, temporal_graph: TemporalGraph, uniform=False, seed=None):
        adj_list = torch.stack(
            [temporal_graph.src, temporal_graph.tgt, temporal_graph.t], dim=1)
        return cls(
            adj_list=adj_list,
            uniform=uniform,
            seed=seed
