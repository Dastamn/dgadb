...
class GraphEmbedding(EmbeddingModule):
    ...
    def compute_recovery_memory_embedding(
        self, memory, source_nodes, timestamps, neighbors, neighbors_time, n_layers, n_neighbors=20
    ):
        assert n_layers >= 0
        ...
        mem_recovery_embedding = self.aggregate(
            n_layers, source_nodes_time_embedding, mem_neighbor_embeddings, edge_time_embeddings, mask
        )
        return mem_recovery_embedding

    def aggregate(self, n_layers, source_nodes_time_embedding, neighbor_embeddings, edge_time_embeddings, mask):
        return None
...
class GraphAttentionEmbedding_recovery(GraphEmbedding):
    def __init__(
        ...
        super(GraphAttentionEmbedding_recovery, self).__init__(
            memory,
            neighbor_finder,
            time_encoder,
            n_layers,
    ...
    def aggregate(self, n_layer, source_nodes_time_embedding, neighbor_embeddings, edge_time_embeddings, mask):
        attention_model = self.attention_models
        source_embedding, _ = attention_model(
            source_nodes_time_embedding, neighbor_embeddings, edge_time_embeddings, mask
        )
...
def get_embedding_module(
    ...
    if module_type == "graph_attention_recovery":
        return GraphAttentionEmbedding_recovery(
            memory=memory,
