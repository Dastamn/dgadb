...
from .time_encoding import TimeEncode, Fixed_time_encode
...
class SLADE_TGN(torch.nn.Module):
    def __init__(self, neighbor_finder, device, n_nodes, n_edges, n_layers=1,
                 ...
                 src_reg_factor: int | float = 1, dst_reg_factor: int | float = 1, only_drift_loss=False, only_recovery_loss=False):
        ...
        # first node memory is empty (because of zero neighbor masks)
        ...
        self.memory = None
        self.bias_src_memory = nn.Parameter(
            torch.zeros(memory_dimension), requires_grad=False)
        self.bias_dst_memory = nn.Parameter(
            torch.zeros(memory_dimension), requires_grad=False)
    ...
    def compute_temporal_embeddings(self, source_nodes, destination_nodes, edge_times,
        ...
        if test:
            ...
            memory, _ = self.get_updated_memory_tensor(
                all_nodes, self.memory.messages_tensor, self.memory.messages_time)
        else:
            memory, _ = self.get_updated_memory_tensor(torch.arange(
                1, self.n_nodes), self.memory.messages_tensor, self.memory.messages_time)
        ...
        self.update_memory_tensor(
            nodes, self.memory.messages_tensor, self.memory.messages_time)
        self.memory.clear_messages(nodes)

        unique_sources, source_message, source_message_ts = self.get_raw_messages(
            source_nodes, destination_nodes, edge_times)
        unique_destinations, dst_message, dst_message_ts = self.get_raw_messages(
            destination_nodes, source_nodes, edge_times)

        self.memory.store_raw_messages(
            unique_sources, source_message, source_message_ts)
        self.memory.store_raw_messages(
            unique_destinations, dst_message, dst_message_ts)
    ...
    def compute_node_diff_score(self, source_nodes, destination_nodes, edge_times, src_neighbors, dst_neighbors, src_neighbors_time, dst_neighbors_time, n_neighbors, seen_nodes):
        ...
        positive_recovery_score = torch.exp(torch.diag(
            cosine_similarity(pos_node_embedding, node_memory)).reshape(-1))
        negative_recovery_score = torch.exp(cosine_similarity(
            pos_node_embedding, negative_memory)).sum(dim=1)

        positive_drift_score = (torch.exp(torch.diag(
            cosine_similarity(node_memory, prev_memory)).reshape(-1)))
        negative_drift_score = torch.exp(cosine_similarity(
            node_memory, negative_memory)).sum(dim=1)

        dst_positive_recovery_score = torch.exp(torch.diag(
            cosine_similarity(dst_node_embedding, dst_node_memory)).reshape(-1))
        dst_negative_recovery_score = torch.exp(cosine_similarity(
            dst_node_embedding, negative_memory)).sum(dim=1)

        dst_positive_drift_score = (torch.exp(torch.diag(
            cosine_similarity(dst_node_memory, prev_dst_memory)).reshape(-1)))
        dst_negative_drift_score = torch.exp(cosine_similarity(
            dst_node_memory, negative_memory)).sum(dim=1)

        if self.only_drift_loss:      # only memory drift loss
            contrastive_loss = - torch.log(positive_drift_score/(negative_drift_score)) \
                - torch.log(dst_positive_drift_score /
                            (dst_negative_drift_score))
            ...
            contrastive_loss = - torch.log(positive_recovery_score/(negative_recovery_score))\
                - torch.log(dst_positive_recovery_score /
                            (dst_negative_recovery_score))
        else:                         # original
    ...
    def compute_anomaly_score(self, source_nodes, destination_nodes, edge_times, src_neighbors, dst_neighbors, src_neighbors_time, dst_neighbors_time, n_neighbors):
        ...
        positive_recovery_score = torch.diag(cosine_similarity(
            pos_node_embedding, node_memory)).reshape(-1)
        dst_positive_recovery_score = torch.diag(cosine_similarity(
            dst_node_embedding, dst_node_memory)).reshape(-1)
        positive_drift_score = torch.diag(
            cosine_similarity(node_memory, prev_memory)).reshape(-1)
        dst_positive_drift_score = torch.diag(cosine_similarity(
            dst_node_memory, prev_dst_memory)).reshape(-1)
    ...
    def get_raw_messages(self, source_nodes, destination_nodes, edge_times):
        ...
        source_message = torch.cat(
            [destination_memory, source_time_delta_encoding], dim=1)
