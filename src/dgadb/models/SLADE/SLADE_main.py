...
class SLADEModel:
    ...
    def __init__(
        ...
        logger.info(f"Initializing SLADEModel with device={self.device} and hyperparams={hyperparams}")
        ...
        self.negative_memory_type = hyperparams.get("negative_memory_type", "train")
        ...
        self.only_drift_loss_score = hyperparams.get("only_drift_loss_score", False)
        self.only_recovery_loss_score = hyperparams.get("only_recovery_loss_score", False)
        self.only_drift_score = hyperparams.get("only_drift_score", False)
        self.only_rec_score = hyperparams.get("only_rec_score", False)
        self.test_inference_time = hyperparams.get("test_inference_time", False)
    ...
    def setup(self, df: pd.DataFrame) -> None:
        ...
        self.full_data = SLADEData(sources, destinations, timestamps, edge_idxs, labels)
        ...
        self.train_ngh_finder = get_neighbor_finder(self.train_data, uniform=False, max_node_idx=max_idx)
        if self.has_val:
            self.val_ngh_finder = get_neighbor_finder(self.train_val_data, uniform=False, max_node_idx=max_idx)
            # should be safe based on how neighbor finder is implemented
        self.full_ngh_finder = get_neighbor_finder(self.full_data, uniform=False, max_node_idx=max_idx)
        ...
        self.train_data_sources = torch.from_numpy(self.train_data.sources).long().to(self.device)
        self.train_data_destinations = torch.from_numpy(self.train_data.destinations).long().to(self.device)
        self.train_data_timestamps = torch.from_numpy(self.train_data.timestamps).float().to(self.device)
        self.train_data_src_neighbors = torch.from_numpy(self.src_neighbors).long().to(self.device)
        self.train_data_dst_neighbors = torch.from_numpy(self.dst_neighbors).long().to(self.device)
        self.train_data_src_neighbors_time = torch.from_numpy(self.src_neighbors_time).long().to(self.device)
        self.train_data_dst_neighbors_time = torch.from_numpy(self.dst_neighbors_time).long().to(self.device)
        ...
        self.negative_train_nodes = (
            torch.from_numpy(np.array(list(set(self.train_data.destinations) | set(self.train_data.sources))))
            .long()
            .to(self.device)
        )
    ...
    def inference(self, split: str) -> tuple[np.ndarray, np.ndarray, float]:
        ...
        if split == "train":
            ...
            logger.warning("Tried to evaluate on validation split, but none was found.")
