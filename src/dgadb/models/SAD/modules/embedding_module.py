...
from src.dgadb.models.SAD.modules.temporal_attention import TemporalAttentionLayer2
from src.dgadb.models.SAD.modules.temporal_sum import TemporalSumLayer


class EmbeddingModule(nn.Module):
    def __init__(
        self, time_encoder, n_layers, node_features_dims, edge_features_dims, time_features_dim, hidden_dim, dropout
    ):
...
class GraphEmbedding(EmbeddingModule):
    def __init__(
        ...
        super(GraphEmbedding, self).__init__(
            time_encoder, n_layers, node_features_dims, edge_features_dims, time_features_dim, hidden_dim, dropout
        )
    ...
    def compute_embedding(self, neigh_edge, edge_to_time, edge_feat, node_feat, edge_mask=None, sample_ratio=None):
        ...
        for layer in range(n_layers):
            temp_node_feat = self.aggregate(
                n_layers, temp_node_feat, neigh_edge, edge_feat, src_time_embeddings, edge_time_embeddings
            )

        out = temp_node_feat
        return out

    def aggregate(self, n_layers, node_features, edge_index, edge_feature, src_time_features, edge_time_embeddings):
        return None
...
class GraphSumEmbedding(GraphEmbedding):
    ...
    def aggregate(self, n_layer, node_features, edge_index, edge_feature, src_time_features, edge_time_embeddings):
        attention_model = self.attention_models[n_layer - 1]

        source_embedding = attention_model(
            node_features, edge_index, edge_feature, src_time_features, edge_time_embeddings
        )
