...
class TemporalAttentionLayer2(torch.nn.Module):
    ...
    def __init__(
        self, n_node_features, n_neighbors_features, n_edge_features, time_dim, output_dimension, n_head=2, dropout=0.1
    ):
    ...
    def forward(self, node_feature, edge_index, edge_feature, src_time_features, edge_time):
        ...
        if self.reverse_flag:
            edge_index, edge_feature, src_time_features, edge_time = self.reverse_edge(
                edge_index, edge_feature, src_time_features, edge_time
            )
        if self.selfloop_flag:
            edge_index, edge_feature, src_time_features, edge_time = self.add_selfloop(
                node_feature, edge_index, edge_feature, src_time_features, edge_time
            )
        node_i = edge_index[:, 0]
        node_j = edge_index[:, 1]
