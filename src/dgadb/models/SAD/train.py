...
def criterion(prediction_dict, labels, model, config):
    ...
    if config.mode == "sad":
        loss_anomaly = model.gdn.dev_loss(
            torch.squeeze(labels), torch.squeeze(prediction_dict["anom_score"]), torch.squeeze(prediction_dict["time"])
        )
...
def main():
    ...
    # log and path
    get_checkpoint_path = (
        lambda epoch: f"{log_base_path}saved_checkpoints/{args.data_set}-{args.mode}-{args.module_type}-{args.mask_ratio}-{epoch}.pth"
    )
