...
class DygDataset(torch.utils.data.Dataset):
    def __init__(self, graph, split_flag, n_layer, n_neighbors, mask_label, mask_ratio):
        self.n_layer = n_layer
        self.n_neighbors = n_neighbors
        self.mask_label = mask_label
        self.mask_ratio = mask_ratio

        self.full_data, self.positive_eids, self.edge_features, self.node_features = self.get_data(graph, split_flag)
        ...
        if split_flag == "train" and self.mask_label:
            self.full_data = self.masked_edge_label(self.full_data, self.positive_eids, self.mask_ratio)
    ...
    def get_data(self, graph, split_flag):
        ...
        if split_flag == "train":
            positive_eids = train_eids
            pass
        elif split_flag == "val":
            positive_eids = val_eids
            pass
        elif split_flag == "test":
            positive_eids = test_eids
            pass
    ...
    def up_sample_data(self, full_data, positive_eids, mask_ratio):
        ...
        pickup_index = np.random.choice(
            black_upsample_numbers, size=int(len(black_upsample_numbers) / (1 - mask_ratio)), replace=True
        )
        # 随机插入的位置
        ...
        new_full_data = SADData(new_sources, new_destinations, new_timestamps, new_edge_idxs, new_labels)
