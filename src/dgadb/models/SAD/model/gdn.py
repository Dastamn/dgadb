...
class AnomalyLayer(torch.nn.Module):
    def __init__(self, dim):
        ...
        self.config = [("linear", [dim, dim]), ("linear", [1, dim])]
...
class graph_deviation_network(torch.nn.Module):
    def __init__(self, arg_dict, device):
        ...
        self.net = AnomalyLayer(arg_dict["hidden_dim"])
        self.memory_size = arg_dict["memory_size"]  # 5000 # 4000
        self.sample_size = arg_dict["sample_size"]  # 2000 # 1000
        self.device = device
        self.memory = torch.randn_like(torch.zeros(self.memory_size, dtype=torch.float32, requires_grad=False)).to(
            device
        )
    ...
    def dev_loss(self, y_true, y_prediction, current_time):
        ...
        outlier_loss[outlier_loss < 0.0] = 0
    ...
    def forward(self, x, time, label=None):
        ...
        if self.training:
            ...
            if self.idx_after < self.memory_size:
                ...
                self.time_memory[self.idx : self.memory_size] = record_time_mem.reshape([-1])[
                    0 : (self.memory_size - self.idx)
                ]
