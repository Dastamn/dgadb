...
def run_experiment(
    ...
    anom_types: Annotated[List[str], typer.Option(help="Anomaly types to inject. Choices: random, burst, bridge, clique, path.")] = ["random", "burst", "bridge", "clique", "path"],
    anom_rates: Annotated[List[float], typer.Option(help="Anomaly injection rate(s) R as a fraction of the val/test split size, e.g. 0.05 = 5%.")] = [0.1, 0.05, 0.01],
    anom_durations: Annotated[List[str], typer.Option(help="Temporal-span buckets T over which an anomaly group is spread. Choices: small (0.1), medium (0.5), large (1.0).")] = ["small", "medium", "large"],
