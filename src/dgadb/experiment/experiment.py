...
class Experiment():
    ...
    def preprocessing(self, flip_labels:bool):
        pipeline, meta_dict = Pipeline.from_config(self.dataset_name, force_rerun=True)
        ...
        self.graph = convert_temporal_graph_to_legacy_graph(anomalous_temporal_graph)
    ...
    def training_function(self, config: dict, model_class: Type[RustGraphModel], graph: Graph, meta_dict: dict, *args):
        ...
        graph.generate_snapshots(snapshot_size=config["snapshot_size"], temporal_snapshots=False)
