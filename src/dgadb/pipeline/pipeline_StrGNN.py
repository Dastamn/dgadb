...
if __name__ == "__main__":
    ...
    pipeline,_,_ = Pipeline.from_config(config_name, force_rerun=False)
    ...
    anomalous_temporal_graph = inject_anomalies_addgraph_style(
        temporal_graph, anom_train_ratio=0.5, anom_val_ratio=0.05, anom_test_ratio=0.05, noise_ratio=0.0)
    ...
    model = STRGNNModel(device, meta_dict, hyperparams, roc_auc_score)

    start_setup = time()
    model.setup(anomalous_temporal_graph)
    ...
    preds, labels = model.inference(split="val")

    total_auc_score = roc_auc_score(labels, preds)
    logger.info(f"Total val score: {total_auc_score:.4f}")
