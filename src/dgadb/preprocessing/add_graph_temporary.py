...
def inject_anomalies_addgraph_style(
    ...
    def _make_split(mask: torch.Tensor, ratio: float):
        ...
        fake = torch.tensor(fake_pairs[:num_anom], dtype=tg.src.dtype, device=device)

        total = m + num_anom
        labels = torch.ones(total, dtype=torch.long, device=device)  # 1 = normal
        ...
        out_msg = None
        if fdim is not None:
            out_msg = torch.empty(total, fdim, dtype=tg.msg.dtype, device=device)
    ...
    val_pack   = _make_split(tg.val_mask   if tg.val_mask is not None else None, anom_val_ratio)
    ...
    new_lab  = torch.cat([p[4] for p in parts])  # 1=normal, 0=anomaly (original)

    sizes = [p[0].numel() for p in parts]
    split_names = []
    if train_pack is not None: split_names.append("train")
    if val_pack   is not None: split_names.append("val")
    if test_pack  is not None: split_names.append("test")
    ...
    val_mask   = torch.zeros_like(new_lab, dtype=torch.bool) if tg.val_mask is not None else None
    ...
    return TemporalGraph(
        ...
        node_labels=(tg.node_labels.clone() if tg.node_labels is not None else None),
        metadata=meta,
    )